...
class VariantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create variant" do
    assert_difference("Variant.count") do
      post variants_url, params: { variant: { name: @variant.name, unit_id: @variant.unit_id } }
    end
  ...
  test "should update variant" do
    patch variant_url(@variant), params: { variant: { name: @variant.name, unit_id: @variant.unit_id } }
