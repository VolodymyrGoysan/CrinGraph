...
import Equalizer from "../../../lib/equalizer"; // TODO: add local path resolving

export default function useGraphBox(config) {
  ...
  const padding = { left: 15, right: 15, top: 10, bottom: 36 };
  const W0 = 800;
  const H0 = 360;
  const W = W0 - padding.left - padding.right;
  const H = H0 - padding.top - padding.bottom;

  useEffect(() => {
    const doc = d3.select(".graphtool");
    const graph = doc.select("#fr-graph");
    ...
    let yR = [padding.top + H, padding.top + 10];
    ...
    dB.all = graph.append("g").attr("class", "dBScaler"),
    ...
    function clearLabels() {
      graph.selectAll(".lineLabel").remove();
    ...
    function drawLabels() {
      ...
      graph.selectAll(".lineLabel").remove();
      let g = graph.selectAll(".lineLabel").data(bcurves)
    ...
    function saveGraph(ext) {
      ...
      drawLabels();
      showControls(false);
      fn(graph.node(), "graph." + ext, { scale: 3 })
    ...
    let gpath = graph.insert("g", ".dBScaler")
      .attr("fill", "none")
    ...
    let norm_sel = (config.notmalizationType.toLowerCase() === "db") ? 0 : 1,
      norm_fr = config.normalizationHz,
      norm_phon = config.normalizationDb;
    ...
    function pathTooltip(c, m) {
      let g = graph.selectAll(".lineLabel").data([c.id])
    ...
    let graphInteract = imm => function () {
      ...
      if (interactInspect) {
        ...
        let insp = graph.selectAll(".inspector").data([xv])
        ...
        let d = 30 * W0 / graph.node().getBoundingClientRect().width,
    ...
    function stopInspect() { graph.selectAll(".inspector").remove(); }
    graph.append("rect")
    ...
    function addHeader() {
      ...
      headerLinks.forEach(function (link) {
        ...
        if (config.altHeaderNewTab) { linkElem.setAttribute("target", "_blank"); }
    ...
    if (config.altLayout && config.altHeader) { addHeader(); }
    ...
    if (config.altTutorial) { addTutorial(); }
    ...
    function toggleExpandCollapse() {
      ...
      if (graphIsIframe && config.expandableOnly) {
        const expandOnlyMax = config.expandableOnly === true ? 1000000 : config.expandableOnly;
